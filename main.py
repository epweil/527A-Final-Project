...
class CustomPromptTemplate(StringPromptTemplate):
    ...
    def format(self, **kwargs) -> str:
        ...
        tool_strings = []
        if last_action and last_action.tool == VIEW_DEBATE and count_debate < max_debate:
            # Include only view_debate
            for tool in self.tools:
                if tool.name == VIEW_DEBATE:
        ...
        elif last_action and last_action.tool == VIEW_DEBATE:
            # Exclude view_debate and reset count_debate if max is reached
            for tool in self.tools:
                if tool.name != VIEW_DEBATE:
                    s = f"{tool.name}: {tool.description}"
                    params = [f'{param} - {info["description"]}' for param, info in tool.args.items()]
                    s += ' Arguments: ' + ' '.join(params)
                    tool_strings.append(s)
            count_debate = 0  # Reset count_debate
        else:
            for tool in self.tools:
                s = f"{tool.name}: {tool.description}"
                params = [f'{param} - {info["description"]}' for param, info in tool.args.items()]
        ...
        try:
            if match:
                ...
                try:
                    ...
                    raise ValueError(
                        f"Could not parse the JSON string describing the tool: `{tool_info_str}`")
                ...
                raise ValueError(
                    f"Could not find text surrounded by 3 backticks: `{llm_output}`")
        except:
...
def majority_vote(actions):
    ...
    tied_actions = [action for action, count in most_common if count == max_votes]
    return random.choice(tied_actions)

api_key = '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'
...
for _ in range(num_tasks):
    ...
    agent_executor = AgentExecutor.from_agent_and_tools(
        agent=agent, tools=tools, verbose=True, max_iterations=2*MAX_STEPS + 1)
    ...
    for step_num, step in enumerate(agent_iterator):
        ...
        # intermediate_step is a list of (action, observation) pairs
